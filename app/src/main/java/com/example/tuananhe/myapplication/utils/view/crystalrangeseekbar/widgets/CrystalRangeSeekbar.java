package com.example.tuananhe.myapplication.utils.view.crystalrangeseekbar.widgets;
...
import com.example.tuananhe.myapplication.utils.view.crystalrangeseekbar.interfaces.OnRangeSeekbarChangeListener;
import com.example.tuananhe.myapplication.utils.view.crystalrangeseekbar.interfaces.OnRangeSeekbarFinalValueListener;
...
public class CrystalRangeSeekbar extends View {
    ...
    protected void setupBar(final Canvas canvas, final Paint paint, final RectF rect) {
        ...
        if (barColorMode == CrystalSeekbar.ColorMode.SOLID) {
            ...
            rect1.left = rect.left - getThumbWidth()/2;
            rect1.bottom = rect.bottom;
            rect1.top = rect.top;
            rect1.right = normalizedToScreen(normalizedMinValue) + (getThumbWidth() / 2);
            paint.setColor(barColor);
            drawBar(canvas, paint, rect1);

            RectF rect2 = new RectF();
            rect2.left = normalizedToScreen(normalizedMaxValue) + (getThumbWidth() / 2);
            rect2.bottom = rect.bottom;
            rect2.top = rect.top;
            rect2.right = rect.right;
            paint.setColor(barColor);
            drawBar(canvas, paint, rect2);
        }
    }
    ...
    private void setMaxStartValue() {
        if (maxStartValue <= absoluteMaxValue && maxStartValue > absoluteMinValue && maxStartValue >= absoluteMinStartValue) {
            ...
            setNormalizedMaxValue(absoluteMaxValue);
        }
    }
